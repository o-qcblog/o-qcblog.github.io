...
A very naïve way to get to the number fast is by trying to rule out a large chunk of possibilities. Obviously, asking questions like “Is the number =1?”, “Is it =2?” and so on is not so good. Here, you only rule out 1 number at a time. You would need 100 steps if 100 is the number. So a strategy that gets rid of a large portion of possible numbers with every subsequent question seems to be great.
